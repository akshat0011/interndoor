...
/* Filled from the .trk-mount element src/pages.js renders into the side rail.
   ...
   The control itself is IDTrack.strip — the same one the board's detail pane
   renders, so the two cannot drift on what a status ladder or a note field is. */
(function tracker() {
  ...
  mount.append(strip);
  // One subscription, for the life of the page. Also repaints for a change
  // made in another tab.
  T.on(strip.repaint);
}());
