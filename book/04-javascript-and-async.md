...
- **`setTimeout(() => controller.abort(), TIMEOUT_MS)`** — `TIMEOUT_MS` is `30_000` (`src/gemini.js:20`): generous for a language-model call, short enough that a hourly run is never left hanging. The arrow is a closure over `controller`, and each batch gets a fresh one.
...
3. **Nothing is waiting.** A background job, every hour, on maybe forty postings: seven batches at a few seconds each is half a minute, with nobody watching a spinner.
